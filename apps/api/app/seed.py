...
def ensure_users(db: Session):
    ...
    if not main:
        pwd_hash = settings.main_password_hash or hash_password(settings.main_password or 'change-me-now')
        db.add(User(username=settings.main_username, password_hash=pwd_hash, role='owner', account_type='main'))
    demo = db.query(User).filter(User.username == settings.demo_username).first()
    if not demo:
        pwd_hash = settings.demo_password_hash or hash_password(settings.demo_password)
        demo = User(username=settings.demo_username, password_hash=pwd_hash, role='demo', account_type='demo')
        db.add(demo); db.commit(); seed_demo_data(db, demo)
    db.commit()
