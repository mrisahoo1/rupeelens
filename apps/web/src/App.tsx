...
function Protected() { const {data, isLoading, isError} = useQuery({queryKey:['me'], queryFn: api.me, retry:false, enabled: Boolean(tokenStore.get())}); if (!tokenStore.get()) return <Navigate to="/login"/>; if (isLoading) return <div className="grid min-h-screen place-items-center text-white/60">Opening RupeeLens...</div>; if (isError || !data) return <Navigate to="/login"/>; return <Layout user={data}/>; }
